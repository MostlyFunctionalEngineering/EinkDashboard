...
def render(epd, config):
    ...
    try:
        ...
        time_w, time_h = font.getmask(time_str).size
        time_x = (height - time_w) // 2
        time_y = (width // 2 - time_h) if not show_date else (width // 3 - time_h // 2)
        draw_black.text((time_x, time_y), time_str, font=font, fill=0)

        # Measure and center date if needed
        if date_str:
            date_w, date_h = font.getmask(date_str).size
            date_x = (height - date_w) // 2
            date_y = (2 * width // 3 - date_h // 2)
